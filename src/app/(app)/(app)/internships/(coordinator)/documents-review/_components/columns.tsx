...
export const DocumentReviewColumns: ColumnDef<ColumnSchema>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge
          variant={getStatusVariant(reviewStatus as statusType)}
          className={cn(
            "rounded-full border px-3 py-1 text-xs font-medium capitalize transition-all duration-200",
            getStatusColor(reviewStatus as statusType),
          )}
        >
          {reviewStatus === "pending" ? (
            <Clock className={cn("h-4 w-4", getStatusColor(reviewStatus))} />
          ) : reviewStatus === "approved" ? (
            <CheckCircle
              className={cn("h-4 w-4", getStatusColor(reviewStatus))}
            />
          ) : (
